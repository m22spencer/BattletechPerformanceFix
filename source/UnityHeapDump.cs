...
// adapted from https://github.com/Zuntatos/UnityHeapDump
...
public class UnityHeapDump
{
  ...
  static String dumpDir;
    ...
    dumpDir = $"dump_{DateTime.Now.ToString("yyyyMMdd_HHmmssfff")}";
		Directory.CreateDirectory(dumpDir);
		Directory.CreateDirectory(dumpDir + "/sobjects");
		...
		using (var logger = new StreamWriter(dumpDir + "/log.txt"))
		{
			...
			Dictionary<Assembly, long> assemblySizes = new Dictionary<Assembly, long>();
			...
			foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
			{
				...
				{
					assemblyFolder = string.Format(dumpDir + "/statics/{0}/", assembly.FullName.Replace("<", "(").Replace(">", ")").Replace(".", "_"));
				} else
				{
					assemblyFolder = dumpDir + "/statics/misc/";
				}
			...
			foreach (var genericType in genericTypes.ToList())
			{
				try
				{
					assemblyResults[genericType.Assembly].Add(new StructOrClass(genericType, dumpDir + "/statics/misc/"));
				}
				catch (Exception e)
				{
			...
			foreach (var pair in assemblyResults)
			{
				...
				pair.Value.Sort((a, b) => (int) (b.Size - a.Size));
			}
      ...
      while (ThreadJobsDone < ThreadJobsPrinting) {
        System.Threading.Thread.Sleep(100);
			...
			assemblySizesList.Sort((a, b) => (int) (b.Value - a.Value));
      long assemblySizesListSize = assemblySizesList.Sum(a => a.Value);

			unityComponents.Sort((a, b) => (int) (b.Size - a.Size));
			long unityComponentsSize = unityComponents.Sum(a => a.Size);
			bool printedUnityComponents = false;

			unityScriptableObjects.Sort((a, b) => (int) (b.Size - a.Size));
			long unityScriptableObjectsSize = unityScriptableObjects.Sum(a => a.Size);
			bool printedUnityScriptableObjects = false;

			logger.WriteLine("Total tracked memory (including duplicates, so too high) = {0}", assemblySizesListSize + unityComponentsSize + unityScriptableObjectsSize);
